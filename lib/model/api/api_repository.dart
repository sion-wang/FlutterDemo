...
class ApiRepository {
  String baseUrl;
  Client httpClient = HttpClientWithInterceptor.build(interceptors: [LoggingInterceptor()], requestTimeout: Duration(seconds: 30));
  ...
  Future<ApiResult<List<User>>> fetchUsers({int since = 0, int perPage = 20}) async {
    try {
      final response = await httpClient.get(
        Uri.https(baseUrl, "/users", {'since': since.toString(), 'per_page': perPage.toString(),}),
